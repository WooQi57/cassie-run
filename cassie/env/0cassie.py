...
class CassieEnv(gym.Env):
    def __init__(self, simrate=60, dynamics_randomization=True,
                 learn_gains=False, visual=True,
                 config="./model/cassie.xml", history=0, **kwargs):
    ...
    def reset(self):
        # print('----reset----')
        self.speed = 4.0 # np.random.uniform(self.min_speed, self.mheight = self.qpos[2]ax_speed)
    ...
    def compute_reward(self, action):
        ...
        self.rew_height = 0.4 * height
        self.rew_joint = 0.2 * np.exp(-joint_penalty)
        self.rew_vel = 0.4 * np.exp(-vel_penalty)
        reward = self.rew_height + self.rew_joint + self.rew_vel
